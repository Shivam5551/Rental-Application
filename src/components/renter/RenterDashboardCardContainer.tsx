    ...
    return (
        <div className="grid gap-3 grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 mt-4">
            ...
            {renterStats.totalSpent > 0 && (
                <DashboardCard
                    heading="Total Spent"
                    value={formatCurrency(renterStats.totalSpent / 100)}
                >
                    <Icons color="bg-emerald-300">
    ...
    return (
        <div className="grid gap-3 grid-cols-1 md:grid-cols-2 lg:grid-cols-3 mt-4">
            {[1, 2, 3].map((i) => (
                <div
                    key={i}
                    className="bg-white rounded-xl shadow-md border p-2 dark:bg-slate-800 animate-pulse"
                >
