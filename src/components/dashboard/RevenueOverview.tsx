...
export const RevenueOverview = () => {
    ...
    return (
        <div className="flex w-full bg-neutral-50 dark:bg-slate-800 mt-4 rounded-xl shadow-md border p-4 flex-col justify-start">
            ...
            <SubHeading
                className="text-left dark:text-red-100"
                title={`Your property revenue Jan ${new Date().getFullYear() - 1} - ${month} ${new Date().getFullYear()}`}
            />
            <div className="flex gap-4 w-full md:flex-row flex-col">
                ...
                <div className="border w-full md:w-[35%] bg-white dark:bg-slate-800/90 dark:border-gray-300 p-6 space-y-4 rounded-2xl">
                    ...
                    <RevenueDetailsComponent title="Total Profit" amount={1300}>
                        <Icons color="bg-green-400">
                            <GiReceiveMoney size={35} />
                        </Icons>
                    </RevenueDetailsComponent>
                    <RevenueDetailsComponent title="Total Expense" amount={100}>
                        <Icons color="bg-blue-300">
                            <GiPayMoney size={35} />
                        </Icons>
                    </RevenueDetailsComponent>
                    <button className="w-full flex items-center justify-center p-4 gap-1 font-medium duration-500 transform transition-all cursor-pointer hover:text-red-800 hover:bg-red-300 rounded-lg hover:rounded-2xl text-red-600 bg-red-200">
                        <FaDownload />
