...
export const UserPropertiesCard = async () => {
    ...
    const property: Property[] = Array.isArray(propertyData)
        ? propertyData.flatMap((item) => item.properties ?? [])
        : [];
    ...
    return (
        <div className="w-full min-h-180 max-h-180 rounded-2xl shadow-md items-center dark:bg-slate-800 p-4 bg-neutral-100 overflow-x-auto">
            ...
            {property.length === 0 ? (
                <div className="text-center text-gray-700 font-semibold flex items-center justify-center w-full min-h-50 text-2xl">
                    No properties found.
                </div>
            ) : (
                <div className="grid p-2 grid-cols-1 xl:grid-cols-2 gap-4">
                    {property.map((prop) => (
                        <div
                            key={prop.id}
                            className="bg-white dark:text-white dark:bg-black relative rounded-lg shadow-lg p-4 flex flex-col"
                        >
                            {prop.discount ? (
                                <div className="absolute bg-red-600 items-center flex justify-center text-white h-12 w-12 font-semibold rounded-full right-2 top-2">
                                    {"-" + prop.discount / 100 + "%"}
                                </div>
                            ) : (
                                ""
                                ...
                                {prop.discount ? (
                                    <div className="flex items-center">
                                        <span
                                            ...
                                            className="text-base mr-2 whitespace-pre items-center font-light"
                                        >
                                            {" Rs." + (prop.price / 100).toFixed(2) + " "}
                                        </span>
                                        <span>
                                            Rs.
                                            {(
                                                (prop.price / 100) *
                                                (1 - prop.discount / 10000)
                                            ).toFixed(2)}
                                        </span>
                                    </div>
                                ) : (
                                    <span>{"Rs." + (prop.price / 100).toFixed(2)}</span>
                                )}
                            </div>
                            <div className="flex items-center mt-2 space-x-2">
                                <span
                                    className={`text-xs px-2 py-1 rounded ${prop.petfriendly ? "bg-green-100 text-green-700" : "bg-red-100 text-red-800"}`}
                                >
