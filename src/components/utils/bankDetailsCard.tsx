    ...
    useEffect(() => {
        async function submitRequest() {
            if (formData.accountNumber && formData.ifscNumber) {
                try {
                    const res = await axios.post("/api/update/bank", formData);
                    if (res.data.success) {
    ...
    if (status === "loading") {
        return (
            <div className="min-w-screen bg-white dark:bg-slate-700 min-h-screen flex justify-center items-center flex-col">
                ...
                <div className="text-gray-700 dark:text-gray-100 text-balance sm:text-2xl ">
                    Authenticating...
                </div>
            </div>
        );
        ...
        <div className="rounded-2xl flex bg-neutral-100 shadow-lg dark:bg-slate-800 items-center justify-center w-full h-fit p-5 flex-col">
            {!!!updating ? (
                <button
                    ...
                    className="text-xl p-2 hover:underline hover:text-blue-500 dark:text-white  cursor-pointer"
                >
                    Update Bank Details
                </button>
            ) : (
                <div className="w-full">
                    <form action={submitForm} className="p-2 w-full space-y-2">
                        <LabelledInputBox
                            labelClassName="dark:text-black text-white dark:bg-white bg-black"
                    ...
                    <div className="px-2 w-full">
                        <button
                            ...
                            className="text-white bg-red-600 transform duration-200 transition-all rounded-xl text-lg font-semibold hover:rounded-2xl  hover:bg-red-400 w-full p-2 flex justify-center items-center cursor-pointer"
                        >
                            Cancel
                        </button>
                    </div>
