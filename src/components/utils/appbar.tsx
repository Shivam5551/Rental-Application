        ...
        <nav className="fixed top-0 w-full bg-white/80 backdrop-blur-sm z-50 border-b dark:border-b-neutral-200  dark:bg-black/80 p-3 shadow-md ">
            <div className=" flex px-2 justify-between items-center">
                ...
                <div className="flex items-center transform duration-500 transition-colors ease-in-out">
                    <div
                        id="menu-toggle"
                        className="xl:hidden flex items-center justify-center w-10 h-10 rounded-full bg-gray-200 dark:bg-gray-800 text-black dark:text-white hover:cursor-pointer hover:bg-gray-300 dark:hover:bg-gray-700"
                    >
                        <MenubarToggle />
                    </div>
                    <div
                        id="menubar"
                        className="hidden xl:flex flex-col space-y-2 sm:space-y-0 xl:flex-row items-center"
                    >
                        ...
                        {!!session && !!session.user && !!session.user.id && (
                            <NavLink
                                href="/bookings"
                                title="My Bookings"
                                symbol={<LuNotebookTabs className="font-extralight" size={22} />}
                            />
                        )}
                        {session && session.user && session?.user?.id ? (
                            <div className="flex flex-col xl:flex-row xl:space-x-1 space-y-1 xl:space-y-0  items-center">
                                <NavLink
                                    href="/reviews/user"
                                    title="My Reviews"
                                    symbol={<MdOutlineRateReview size={22} />}
                                />
                                <NavLink
                                    href="/profile/renter"
                                    title="Profile"
                                    symbol={<CgProfile size={22} />}
                                />
...
const NavLink = ({ href, title, symbol }: { href: string; title: string; symbol: ReactNode }) => {
    return (
        <a
            href={href}
            className="text-black hover:outline w-full p-2 rounded-2xl hover:cursor-pointer dark:text-white hover:text-gray-700 dark:hover:text-gray-300"
        >
